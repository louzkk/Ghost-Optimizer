...
:menu
title Ghost Optimizer // Menu 2.3
mode 105,25
cls
echo.
echo.
...
echo %p%[ %roxo%+%p% %p%]%w% Caso os arquivos estejam sendo usados feche os apps abertos.
echo %p%[ %roxo%+%p% %p%]%w% A limpeza pode demorar dependendo da quantidade de arquivos.
echo %p%[ %roxo%+%p% %p%]%w% Evite minimizar a janela durante o processo.
timeout /t 3 /nobreak >nul
echo.
:: @louzkk
...
echo.
echo %p%[ %roxo%+%p% %p%]%w% Iniciando Otimização Agressiva %p%(%roxo%~4s%p%)%w%...
echo %p%[ %roxo%+%p% %p%]%w% Evite minimizar a janela durante o processo.
echo.
...
echo %p%[ %roxo%+%p% %p%]%w% Plano de Energia de %r%Alto Desempenho%w% ativo.
...
reg add "HKEY_CURRENT_USER\Software\Microsoft\GameBar" /v "AutoGameModeEnabled" /t REG_DWORD /d 0 /f >nul
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Games" /v "Start" /t REG_DWORD /d 3 /f >nul
echo %p%[ %roxo%+%p% %p%]%w% Modo de Jogo do Windows desativado.
